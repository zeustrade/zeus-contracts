...
import "./interfaces/IZusIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/IZusMigrator.sol";

contract ZusMigrator is ReentrancyGuard, IZusMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "ZusMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "ZusMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "ZusMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "ZusMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "ZusMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "ZusMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "ZusMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "ZusMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "ZusMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "ZusMigrator: token not whitelisted");
        require(_tokenAmount > 0, "ZusMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "ZusMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(zusPrice);
        require(mintAmount > 0, "ZusMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "ZusMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "ZusMigrator: invalid tokenA");
            require(tokenB != address(0), "ZusMigrator: invalid tokenB");
        ...
        IZusIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "ZusMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "ZusMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "ZusMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "ZusMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("ZusMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "ZusMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "ZusMigrator: invalid _action");
