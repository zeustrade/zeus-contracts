...
contract EsZusBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esZus;

    constructor(address _esZus) public {
        admin = msg.sender;
        esZus = _esZus;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsZusBatchSender: forbidden");
        _;
    }
        ...
        onlyAdmin
    {
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esZus).transferFrom(msg.sender, _accounts[i], _amounts[i]);

            uint256 nextTransferredCumulativeReward =
                _vester.transferredCumulativeRewards(_accounts[i]).add(_amounts[i]);
            ...
            nextTransferredAverageStakedAmount =
                nextTransferredAverageStakedAmount.mul(totalCumulativeReward).div(nextTransferredCumulativeReward);
